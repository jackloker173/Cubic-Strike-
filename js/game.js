...
(function () {
  ...
  function loadLevel(index) {
    ...
    levelNumberEl.textContent = "KHU VỰC " + level.id + " / " + LEVELS.length;
  ...
  function finishRun(success, collided, points, level) {
    ...
    fireBtn.disabled = false;
    if (success) {
      statusEl.textContent = "MỤC TIÊU BỊ PHÁ HỦY";
      ...
      if (currentLevelIndex < LEVELS.length - 1) {
        ...
        statusEl.textContent = "MỤC TIÊU BỊ PHÁ HỦY — HOÀN THÀNH TẤT CẢ KHU VỰC";
      }
      renderLevelList();
    } else if (collided) {
      statusEl.textContent = "VA CHẠM — MẤT TÊN LỬA";
      statusEl.className = "status status-fail";
    } else {
      statusEl.textContent = "TÊN LỬA BAY QUA — KHÔNG TRÚNG MỤC TIÊU";
      statusEl.className = "status status-fail";
    }
  }
